...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CAS_PARSER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CAS_PARSER_LOG'] or 'warn' if it isn't set.
   */
...
export class CasParser {
   ...
   */
  constructor({
    baseURL = readEnv('CAS_PARSER_BASE_URL'),
    apiKey = readEnv('CAS_PARSER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CasParserError(
        "The CAS_PARSER_API_KEY environment variable is missing or empty; either provide it, or instantiate the CasParser client with an apiKey option, like new CasParser({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CAS_PARSER_LOG'), "process.env['CAS_PARSER_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CAS_PARSER_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static CasParser = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static CasParserError = Errors.CasParserError;
